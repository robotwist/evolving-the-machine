    ...
    <div className="text-left mt-4 p-3 bg-white/5 rounded border border-white/10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-white/90">
        <label className="flex items-center justify-between gap-2">
          <span>Preset</span>
          <select value={preset} onChange={(e) => setPreset(e.target.value as 'custom' | 'performance' | 'quality' | 'mobile' | 'accessibility')} className="bg-black/50 border border-white/20 rounded px-2 py-1">
            {presetOptions.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}
          </select>
        </label>
        <label className="flex items-center justify-between gap-2">
            <span>Voice Style</span>
            <select value={voiceStyle} onChange={(e) => setVoiceStyle(e.target.value as 'normal' | 'haunting' | 'glitch')} className="bg-black/50 border border-white/20 rounded px-2 py-1">
        ...
        <label className="flex items-center justify-between gap-2">
            ...
            <select value={graphicsQuality} onChange={(e) => setGraphicsQuality(e.target.value as 'low' | 'medium' | 'high')} className="bg-black/50 border border-white/20 rounded px-2 py-1">
        ...
        <label className="flex items-center justify-between gap-2">
            <span>FPS Cap</span>
            <select value={fpsCap} onChange={(e) => setFpsCap(Number(e.target.value) as 30 | 60 | 120 | 0)} className="bg-black/50 border border-white/20 rounded px-2 py-1">
